...
# Time analysis - FIX: Include all 4 time segments
...
# Ensure all time segments are present
...
# Customer Segmentation - FIX: Use dataset's max date instead of current_date
...
# Adjusted segmentation thresholds for the dataset timeframe
...
segment_data_list = segment_stats.collect()
for row in segment_data_list:
...
try:
    ...
    # Create time data with all 4 segments
    time_rows = []
    for time_seg in all_times:
        if time_seg in time_data_dict:
            time_rows.append({
                'time_segment': time_seg,
                'conversion_rate': time_data_dict[time_seg]['conversion_rate']
            })
        else:
            time_rows.append({'time_segment': time_seg, 'conversion_rate': 0})
    ...
    bars1 = ax1.bar(labels, funnel, color=['#1f77b4', '#ff7f0e', '#2ca02c'], 
    ...
    ax2.barh(brands_data['brand'][:8][::-1], brands_data['revenue'][:8][::-1], 
             color=plt.cm.viridis(np.linspace(0.2, 0.8, 8)), edgecolor='black', linewidth=1.5)
    ax2.set_title('Top 8 Brands by Revenue', fontweight='bold', fontsize=12)
    ax2.set_xlabel('Revenue ($)')
    ax2.grid(axis='x', alpha=0.3)
    
    # Plot 3: Customer Segments - FIXED
    ax3 = fig.add_subplot(gs[0, 2])
    segment_colors = {'VIP': '#2ca02c', 'Loyal': '#1f77b4', 'Active': '#ff7f0e', 'At-Risk': '#d62728'}
    colors_list = [segment_colors.get(seg, '#gray') for seg in segments_data['segment']]
    wedges, texts, autotexts = ax3.pie(segments_data['count'], 
                                         labels=segments_data['segment'],
                                         autopct='%1.1f%%', 
                                         colors=colors_list,
                                         startangle=90,
    ...
    # Plot 4: Time Conversion - FIXED with all 4 segments
    ax4 = fig.add_subplot(gs[1, 0])
    bars4 = ax4.bar(time_data['time_segment'], time_data['conversion_rate'], 
    ...
    # Plot 5: Event Distribution - FIXED to show all events
    ax5 = fig.add_subplot(gs[1, 1])
    event_types = ['view', 'cart', 'purchase']
    event_counts_list = [event_distribution.get(e, 0) for e in event_types]
    event_labels = [f'{e}\n({c:,})' for e, c in zip(event_types, event_counts_list)]
    ax5.pie(event_counts_list, labels=event_labels, autopct='%1.1f%%',
    ...
    bars6 = ax6.bar(price_data['price_category'], price_data['revenue'], 
    ...
    for bar, v in zip(bars6, price_data['revenue']):
        ax6.text(bar.get_x() + bar.get_width()/2, v, f'${v/1e6:.1f}M', 
                ha='center', va='bottom', fontsize=8)
    ...
    bars7 = ax7.barh(feat_names, feat_values, 
                     color=plt.cm.RdYlGn(np.linspace(0.3, 0.9, 6)),
                     edgecolor='black', linewidth=1.5)
    ax7.set_title('ML Feature Importance (Top 6)', fontweight='bold', fontsize=12)
    ax7.set_xlabel('Importance Score')
    ax7.grid(axis='x', alpha=0.3)
    for bar, v in zip(bars7, feat_values):
        ax7.text(v, bar.get_y() + bar.get_height()/2, f' {v:.1%}',
...
"""
    ...
    print("Resolution: 300 DPI, 20x12 inches\n")
